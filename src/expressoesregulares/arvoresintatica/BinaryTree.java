...
public class BinaryTree {
  ...
  public void executeInOrder(Node node) {
    ...
    switch (node.getSymbol()) {
      case "&":
        NfaMap nfaMap = node.automato.converter.automatoConcatenacao(node.getLeft().automato, node.getRight().automato);
        node.automato = nfaMap;
        break;

      case "*":
        node.automato = node.automato.converter.automatoFechoDeKleene(node.getLeft().automato);
        break;

      case "|":
        if(node.getRight().automato == null){
          node.getRight().automato = node.automato.converter.automatoUnitario(new Regex(node.getRight().getSymbol()));
        }
